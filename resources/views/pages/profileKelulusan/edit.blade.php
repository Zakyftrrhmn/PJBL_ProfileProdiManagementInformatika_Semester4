@extends('layouts.app')
@section('title', 'Edit Data Profile Kelulusan')
@section('content')
    ...
    <div class="flex-none w-full max-w-full px-3">
        <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
            ...
            <div class="flex-auto px-6 pt-5 pb-6">
                <form action="{{ route('profile_kelulusan.update', $profileKelulusan) }}" method="POST">
                    @method('PUT')
                    @csrf

                    <div class="mb-4">
                        <label for="nama_profile" class="block mb-2 text-sm font-medium text-slate-700 dark:text-white">Nama Profile</label>
                        <input type="text" id="nama_profile" name="nama_profile" rows="4" class="block w-full px-4 py-2 text-sm text-slate-700 bg-white border border-slate-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-slate-700 dark:text-white dark:border-slate-600 dark:placeholder-slate-400 dark:focus:ring-blue-500 @error('nama_profile') border-red-500 focus:ring-red-500 dark:border-red-500 dark:focus:ring-red-500 @enderror" required value="{{ $profileKelulusan->nama_profile }}">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea id="deskripsi" name="deskripsi" rows="4" class="block w-full px-4 py-2 text-sm text-slate-700 bg-white border border-slate-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-slate-700 dark:text-white dark:border-slate-600 dark:placeholder-slate-400 dark:focus:ring-blue-500 @error('deskripsi') border-red-500 focus:ring-red-500 dark:border-red-500 dark:focus:ring-red-500 @enderror" required>{{ $profileKelulusan->deskripsi }}</textarea>
                    ...
                    <div class="flex justify-end">
                        <a href="{{ route('profile_kelulusan.index') }}" class="mr-2 text-slate-700 dark:text-white border border-slate-300 hover:bg-slate-100 dark:border-slate-500 dark:hover:bg-slate-700 font-medium rounded-lg text-sm px-4 py-2">
                            Kembali
                        </a>
