        ...
        <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
            ...
            <div class="flex-auto px-6 pt-5 pb-6">
                <form action="{{ route('kalender_akademik.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        <label for="tahun_ajaran" class="block mb-2 text-sm font-medium text-slate-700 dark:text-white">Tahun Ajaran</label>
                        <select id="tahun_ajaran" name="tahun_ajaran" class="block w-full px-4 py-2 text-sm text-slate-700 bg-white border border-slate-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-slate-700 dark:text-white dark:border-slate-600 @error('tahun_ajaran') border-red-500 dark:border-red-500 @enderror" required>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="file" id="file_kalender" name="file_kalender" accept=".pdf,.doc,.docx" class="block w-full text-sm text-slate-700 bg-white border border-slate-300 rounded-lg shadow-sm file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-blue-600 file:text-white hover:file:bg-blue-700 dark:bg-slate-700 dark:text-white dark:border-slate-600 @error('file_kalender') border-red-500 dark:border-red-500 @enderror">
                        @error('file_kalender')
                            <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
