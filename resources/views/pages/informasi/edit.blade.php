                ...
                <form action="{{ route('admin.informasi.update', $informasi->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        <select id="kategori_id" name="kategori_id" required
                            ...
                            @foreach($kategori as $item)
                                <option value="{{ $item->id }}" {{ (old('kategori_id', $informasi->kategori_id) == $item->id) ? 'selected' : '' }}>
                                    {{ $item->nama_kategori }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea id="editor" name="isi" rows="6" required
                            class="block w-full px-4 py-2 text-sm text-slate-700 bg-white border border-slate-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-slate-700 dark:text-white dark:border-slate-600 @error('isi') border-red-500 @enderror">{{ old('isi', $informasi->isi) }}</textarea>
                        @error('isi')
                            <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        @if ($informasi->thumbnail)
                            ...
                            <img src="{{ asset('storage/' . $informasi->thumbnail) }}" alt="Thumbnail Lama" class="mt-2 w-32 h-32 object-cover rounded-lg shadow">
                        @endif
