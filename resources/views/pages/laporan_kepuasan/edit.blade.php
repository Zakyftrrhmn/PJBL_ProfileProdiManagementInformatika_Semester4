@extends('layouts.app')
@section('title', 'Edit Laporan Kepuasan')
@section('content')
    ...
    <div class="flex-none w-full max-w-full px-3">
        <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
            ...
            <div class="flex-auto px-6 pt-5 pb-6">
                <form action="{{ route('admin.laporan_kepuasan.update', $laporan_kepuasan->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-4">
                        ...
                        @if($laporan_kepuasan->file_laporan)
                            <p class="mt-2 text-sm text-slate-600 dark:text-slate-300">
                                File saat ini: <a href="{{ asset('storage/laporan_kepuasan/' . $laporan_kepuasan->file_laporan) }}" target="_blank" class="text-blue-600 underline">
                                    {{ $laporan_kepuasan->file_laporan }}
                                </a>
