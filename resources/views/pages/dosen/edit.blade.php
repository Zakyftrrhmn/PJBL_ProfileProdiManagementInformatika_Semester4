            ...
            <div class="flex-auto px-6 pt-5 pb-6">
                <form action="{{ route('admin.dosen.update', $dosen->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" id="asal_kota" name="asal_kota" value="{{ old('asal_kota', $dosen->asal_kota) }}" required
                               class="block w-full px-4 py-2 text-sm text-slate-700 bg-white border border-slate-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-slate-700 dark:text-white dark:border-slate-600 @error('asal_kota') border-red-500 dark:border-red-500 @enderror">
                        @error('asal_kota')
                            <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" id="nidn" name="nidn" value="{{ old('nidn', $dosen->nidn) }}" required
                               class="block w-full px-4 py-2 text-sm text-slate-700 bg-white border border-slate-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-slate-700 dark:text-white dark:border-slate-600 @error('nidn') border-red-500 dark:border-red-500 @enderror">
                        @error('nidn')
                            <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        @if ($dosen->photo)
                            <p class="mt-2 text-sm text-slate-600 dark:text-slate-300">
                                Foto saat ini: <img src="{{ asset('storage/dosen/' . $dosen->photo) }}" alt="Foto Dosen" class="h-24 mt-2 rounded">
                            </p>
                        @endif
                    </div>
